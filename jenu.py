...
def restart_all_tunnels(debug=False):
    logger.info("Restarting all active SSH tunnels...")
    output = subprocess.check_output("ps aux | grep ssh", shell=True).decode("utf-8").split("\n")
    ssh_tunnels = [line for line in output if "-N -R" in line]

    for tunnel in ssh_tunnels:
        parts = tunnel.split()
        pid = parts[1]
        logger.info(f"Stopping tunnel with PID {pid}")
        stop_tunnel(pid, debug)
        
        # Extract forwarding and VPS info from the process line and restart the tunnel
        match = re.search(r"-N -R\s+(\d+):localhost:(\d+)\s+root@([^\s]+)", tunnel)
        if match:
            remote_port = match.group(1)
            local_port = match.group(2)
            vps = match.group(3)
            key = parts[-1]  # Assume the key is the last part of the command
            logger.info(f"Restarting tunnel with key: {key}, forwarding: {local_port}:{remote_port}, VPS: {vps}")
            open_port(key, f"{local_port}:{remote_port}", vps, debug)

def restart_from_config(file_path, debug=False):
    config = load_config(file_path, debug)
    logger.info(f"Restarting tunnels defined in configuration file: {file_path}")
    ...
    for tunnel in config:
        ...
        existing_tunnel_pids = find_existing_tunnels(key, forwarding, vps, debug)
        for pid, _ in existing_tunnel_pids:
            logger.info(f"Stopping existing tunnel with PID {pid}")
            stop_tunnel(pid, debug)
        
        open_port(key, forwarding, vps, debug)
...
def main():
    ...
    parser.add_argument('--restart', nargs='?', const=True, help='Restart all active tunnels or from a config file')\
    ...
    # Abrir un túnel directamente si se proporcionan key, forwarding, y vps
        ...
        if isinstance(args.restart, str):
            restart_from_config(args.restart, args.debug)
        else:
            restart_all_tunnels(args.debug)
